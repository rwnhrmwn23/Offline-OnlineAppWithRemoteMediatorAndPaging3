...
import com.astro.test.irwan.utils.Mapper.mapToGithubEntities
...
class GithubRemoteMediator(
    ...
    override suspend fun load(
        loadType: LoadType,
        state: PagingState<Int, Github>
    ): MediatorResult {
        var data: List<Github>? = null
        ...
        return try {
            ...
            data?.let {
                localDataSource.insertUsers(it.mapToGithubEntities())
            }
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, Github>): RemoteKeysEntity? {
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()?.let { data ->
            localDataSource.getRemoteKeysId(data.id)
        }
    }

    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, Github>): RemoteKeysEntity? {
        return state.pages.firstOrNull { it.data.isNotEmpty() }?.data?.firstOrNull()?.let { data ->
            localDataSource.getRemoteKeysId(data.id)
        }
    }

    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, Github>): RemoteKeysEntity? {
